...
function RewardsPage() {
  ...
  function openReward(reward) {
    ...
    setSource(reward.costSource === 'cash' ? 'brand' : 'shared')
    setBrandAmount('')
    setResult(null)
  }
  ...
  function handleRedeem(event) {
    ...
    const amount = source === 'split' ? Number(brandAmount) : 0
    const redemption = redeemReward(selectedReward.id, source, { brand: amount, shared: selectedReward.cost - amount })
    setResult(redemption)
    if (redemption.success) setSelectedReward(null)
  }

  const splitSharedAmount = selectedReward ? selectedReward.cost - Number(brandAmount || 0) : 0
  return (
    <div className="rewards-page">
      ...
      <section className="rewards-balances" aria-label="Your balances">
        <div><span>Brand balance</span><strong>{user.rewardBalance}</strong><small>available</small></div>
        <div><span>Shared pool</span><strong>{user.pointsBalance}</strong><small>points available</small></div>
      </section>
      <section className="rewards-catalog"><div className="initiative-section-heading"><div><p className="eyebrow">Shop the collection</p><h2>Choose your reward</h2></div><span>{brand.rewards.length} rewards</span></div>
        {brand.rewards.length === 0 ? <div className="empty-state card">There are no rewards available for this initiative yet.</div> : <div className="reward-grid">{brand.rewards.map((reward) => <article className="reward-card card" key={reward.id}><div className="reward-card__visual"><img src={brand.logo} alt="" /><span aria-hidden="true"><Gift size={23} /></span></div><div className="reward-card__body"><span className="reward-card__source">{reward.costSource === 'either' ? 'Flexible payment' : reward.costSource === 'points' ? 'Points reward' : 'Brand reward'}</span><h3>{reward.name}</h3><p>{reward.description}</p><div className="reward-card__footer"><strong>{reward.cost} <small>{reward.costSource === 'either' ? 'credits' : reward.costSource}</small></strong><button className="button button--primary" type="button" onClick={() => openReward(reward)}>Claim <ArrowRight size={15} aria-hidden="true" /></button></div></div></article>)}</div>}
      </section>
      ...
      {selectedReward && <div className="reward-dialog-backdrop"><form className="reward-dialog card" onSubmit={handleRedeem}><button className="reward-dialog__close" type="button" aria-label="Close reward selection" onClick={() => setSelectedReward(null)}><X size={18} aria-hidden="true" /></button><p className="eyebrow">Ready to claim?</p><h2>{selectedReward.name}</h2><p className="reward-dialog__cost">{selectedReward.cost} {selectedReward.costSource === 'either' ? 'credits' : selectedReward.cost} will be deducted.</p><fieldset className="source-options"><legend>Choose a source</legend>{(selectedReward.costSource === 'either' ? ['brand', 'shared', 'split'] : [selectedReward.costSource === 'cash' ? 'brand' : 'shared']).map((option) => <label key={option} className={source === option ? 'is-selected' : ''}><input type="radio" name="source" value={option} checked={source === option} onChange={() => setSource(option)} />{option === 'brand' ? `Brand balance (${user.rewardBalance})` : option === 'shared' ? `Shared pool (${user.pointsBalance})` : 'Split across both'}</label>)}</fieldset>{source === 'split' && <label className="split-input">Brand balance amount<input type="number" min="0" max={selectedReward.cost} value={brandAmount} onChange={(event) => setBrandAmount(event.target.value)} required /><span>Shared pool will cover {Math.max(0, splitSharedAmount)}.</span></label>}{result?.error && <p className="reward-error" role="alert">{result.error}</p>}<button className="button button--primary" type="submit">Confirm and claim <Check size={15} aria-hidden="true" /></button></form></div>}
    </div>
  )
}
